<template>
  <div class="offer-create-page">
    <section class="pager-sec bfr">
      <div class="container">
        <div class="pager-sec-details">
          <h3>{{ form.id ? 'Edit Offer' : 'Submit Your Free Offer' }}</h3>
        </div>
            ...
            <div class="row">
                                  ...
                                  <div class="row">
                                    <div class="col-xs-12 col-sm-12">
                                      ...
                                      >
                                        ...
                                        >
                                          {{ !isLoggedIn() ? 'Register and Submit your offer' : 'Save' }}
                                        </button>
                                      </div>
                                    </div>
                                  </div>
              ...
              <div class="col-lg-4">
                ...
                <popular-offers-component /> -->
                  ...
                  <!-- END offer-group -->
                  <div class="offer-seller-wrapper" v-if="!isLoggedIn() && !isRegularUser()">
                    <div class="offer-seller">
                      <div class="form-group">
                        <label for="">Register as a supplier</label>
                      </div>
                    </div>
                    <!-- END offer-seller -->
                    <div class="offer-seller-new">
                      <div
                        class="
    ...
    <div
      class="modal modal-save-success fade"
      id="modalSaveSuccess"
      tabindex="-1"
      role="dialog"
      aria-labelledby="modalSaveSuccessLabel"
      aria-hidden="true"
      data-backdrop="static"
    >
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-body">
            <p class="mt-6">
              Your offer has been submitted successfully! We will review it and contact you soon.
            </p>
          </div>
          <div class="modal-footer">
            <button
              type="button"
              class="btn btn-primary"
              @click="handleCloseSaveSuccessDialog"
            >
              Close
            </button>
          </div>
...
export default {
  ...
  created() {
    ...
    const matches = /^\/free-offers\/(\d+)\/edit$/.exec(pathname);
    if (matches) {
      ...
      Promise.all([
        ...
        const [tagResponse, offerResponse] = values;
      ...
      Promise.all([this.fetchTags()]).then((values) => {
        const [tagResponse] = values;
        if (tagResponse.status === 200) {
          const { data } = tagResponse;
          this.tagOptions = data.map((item) => ({
            value: item.id,
  ...
  methods: {
    ...
    async submit() {
      ...
      const submittedUrl = this.form.id
        ...
        : `${window.location.origin}/api/free-offers`;
      ...
      // If a user want to edit a free offer, he needs to be authenticated first
      ...
      axios(config)
        .then((response) => {
          ...
          if (!this.form.id) {
            ...
            this.handleOpenSaveSuccessDialog();
          } else {
            // Open success notification
            this.$store.commit("openNotification", {
